...
class(NTL.phys.data$lakeid) #character
class(NTL.phys.data$depth) #numeric

# dplyr filtering 
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) # filter function from dplyr. filter condition is numeric because the class is numeric
...
Ex4 <-
  Ex3 %>% 
  filter(depth == 0) %>% 
  summarize(mean.surf.temp = mean(temperature_C))
...
Ex5 <-
  ...
  filter(depth == 0) %>% 
  group_by(lakename) %>% 
  summarize(means = mean(temperature_C, na.rm = TRUE))
