...
knitr::opts_chunk$set(echo = TRUE)# this is for knitting purposes. also put the packages here. and you can keep the install.packages part here as well. 
```
...
#Option 1
df <- cbind(df_car_speed,car_dist)  #similarly rows could be added using rbind() #cbind is column binding
class(df)
df
...
tail(df,10) # show the last 10 rows
...
df <- df[,-2] # turns df back into a vector
df
```
